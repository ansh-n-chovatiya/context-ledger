...
[A finished example is in the repository](examples/preview-password-reset.html)
— a password-reset plan of six steps in four rounds, with the plain language
written. Open it in a browser to see what a reviewer is handed before any work
starts. It is a snapshot generated by 0.9.0 and nothing regenerates it, so it
shows that version's output rather than yours.
