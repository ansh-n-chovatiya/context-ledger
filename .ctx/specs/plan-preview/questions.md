...
- [x] Where should a human-readable step title come from, given that slug-derived titles read 'Safe html' and 'Cli wiring'?
- [x] preview.html re-diffs on every plan-check because the view-model quotes plan.json's revision counter. Keep, drop, or replace with the digest?
...
- Where should a human-readable step title come from, given that slug-derived titles read 'Safe html' and 'Cli wiring'? → The plain.md block heading: '## Unit: 01-plain-source - Make the safety check honest'. The title is authored substance and belongs beside the five authored sentences, not in the engineer-facing contract. Falls back to the slug-derived title when absent. Deriving from the objective was correctly refused by unit 03 - it would inject .py paths into the default view and fail the vocabulary check. Requires reopening unit 01 (plain.py parsing) and unit 03 (prefer the authored title); scheduled after unit 04 lands so preview.py is not edited while 04 reads it. (2026-09-12)
- preview.html re-diffs on every plan-check because the view-model quotes plan.json's revision counter. Keep, drop, or replace with the digest? → Drop plan.revision from the page: remove it from the view-model (unit 03) and stop rendering it (unit 04). This makes the page genuinely byte-stable across no-op re-checks, which is what section 3 promises and what the content-digest decision was chosen to achieve. My earlier claim that the digest alone delivered byte-stability was wrong - unit 05 tested it rather than trusting the dispatch note and found the revision is embedded twice, in a footer sentence and in the embedded view-model JSON. The revision remains available in plan.json and through ctx preview --data; the digest identifies the plan's substance in the page. Sequenced after unit 05 gates, because unit 05 now owns the two test files that assert the current behaviour. (2026-09-12)
