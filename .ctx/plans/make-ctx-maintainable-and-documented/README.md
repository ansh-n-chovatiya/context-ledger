...
- `01-shared-paths-and-dead-code` (subagent, pending) — Put the three copies of `LEDGER_PREFIX` and the five hand-built unit-file paths behind the single definitions 
  - owns: ctx/paths.py, ctx/snapshot.py, ctx/worktree.py, ctx/briefing.py, ctx/work.py, ctx/hooks.py, tests/test_shared_paths.py
- `02-caps-and-reported-spend` (subagent, pending) — Give a wave a hard unit cap alongside its token budget, and give the complexity weights a way to be checked ag
...
- `03-verify-dispatch-and-gate` (subagent, pending) — Make a new verify kind one dict entry instead of four coordinated edits, and move the two gate-orchestration f
  - owns: ctx/verify.py, ctx/cli.py, tests/test_verify_kinds_table.py

**Wave 3** — these may run concurrently

- `04-detect-and-advice` (subagent, pending) — Three of the five extractions: the ecosystem detector, the product's decision tree, and the five-way duplicate
  - owns: ctx/detect.py, ctx/advice.py, ctx/cli.py, tests/test_detect.py, tests/test_advice.py

**Wave 4** — these may run concurrently

- `05-commands-registry-and-json` (subagent, pending) — Turn `build_parser` into a registry, then use it to add `--json` in one place instead of forty-one — the pairi
