...
def commands():
    ...
    return (
        ...
        command(
            "preview", "render the plan as a page a non-technical reviewer can read",
            cmd_preview, NAME,
            flag("--data", action="store_true", help="also write preview.data.json"),
            flag("--check", action="store_true",
                 help="check the page on disk against the plan; renders nothing"),
            flag("--open", action="store_true", help="open it in a browser"),
            flag("--scaffold-plain", action="store_true",
                 help="write the plain-language form a human fills in"),
            flag("--force", action="store_true",
                 help="let --scaffold-plain rewrite an existing plain.md"),
            emits_json=True,
        ),
        command(
