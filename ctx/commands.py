...
module-shaped piece out of it — `paths`, the verify kind table, the gate,
the detector, the decision tree — and what was left was still three thousand
lines, because what was left was the command bodies themselves. They are here
now, and `cli.py` is the entry point and the registry: argument parsing, the
table of commands, and `main`.

The split is one-directional and has to stay that way. `cli` imports this
module; this module imports `cli` from nowhere, at module level or inside a
function, because `tests/test_no_import_cycles.py` holds `cli` to being the
sink that nothing imports back. So every helper that both layers need lives
*here* and is re-exported there, rather than the other way round — including
`_ADVISED` and `_RENDER`, which are mutable holders shared by identity: `main`
clears and fills the same objects these bodies read.

Nothing about a command's behaviour changed in the move. The parser surface,
the seven JSON shapes and the prose are all pinned by suites that were not
touched to make this pass.
"""
...
# --------------------------------------------------------------------------- #
# --json: the document a command produces
# --------------------------------------------------------------------------- #
